      ...
      <td className="p-3 text-lg text-gray-800">
        <fetcher.Form method="POST">
          <button
            ...
            name="id"
            value={product.id}
            className={`${isAvailable ? 'text-black': 'text-red-600'} rounded-lg p-2 text-xs uppercase font-bold w-full border border-black hover:cursor-pointer hover:bg-slate-200`}>
          ...
          <Form className="w-full"
            ...
            onSubmit={(event)=> {
              if(!confirm('¿Estás seguro de eliminar este producto?')) {
                event?.preventDefault() // Cancel the form submission and not execute the action
              }
            }}>
            <input type="submit"
              value='Eliminar'
